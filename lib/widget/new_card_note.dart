...
class _NewNoteCardWidgetState extends State<NewNoteCardWidget> {
  ...
  int selectedCol = 0;
  bool _canShowColButton = false;

  // ---------------------------------------------
  // Helpers (มีแค่อันเดียวพอ)
  // ---------------------------------------------
  Map<String, dynamic> safeDecodeMap(dynamic raw) {
    try {
      if (raw == null) return {};
  ...
  void _initFromDrugInstruction() {
    // ✅ ต้องอ่านจาก drug_instruction เท่านั้น
    final setValue = safeDecodeMap(widget.dataNote.drug_instruction);

    final lv = parseStringList(setValue['level']);
    final colFromJson = parseInt(setValue['col'], fallback: 0);

    levelList = lv;
    selectedCol = colFromJson;

    // ถ้ายังไม่เคยเลือก ให้ default เป็นตัวแรก
    if ((selectedLevel == null || selectedLevel!.isEmpty) &&
        levelList.isNotEmpty) {
      selectedLevel = levelList.first;
    }

    // ✅ ปุ่ม COL: ถ้าต้องการให้ขึ้นเสมอเมื่อ Observe (แนะนำ)
    _canShowColButton = (widget.typeCard == 'Observe');

    // ถ้าคุณอยากให้ขึ้น “เฉพาะมี key col” ใช้อันนี้แทน:
    // _canShowColButton = (widget.typeCard == 'Observe' && setValue.containsKey('col'));
  }
  ...
  void initState() {
    ...
    // ✅ init Level/COL จาก drug_instruction
    _initFromDrugInstruction();
  ...
  Color getCardColor() {
    switch (widget.typeCard) {
      ...
      default: // Observe / Special
        return Colors.pink.shade100;
    }
  }

  @override
  void didUpdateWidget(covariant NewNoteCardWidget oldWidget) {
    super.didUpdateWidget(oldWidget);

    // ถ้า dataNote หรือ instruction เปลี่ยน (เช่นสลับ pet/visit) ให้ init ใหม่
    final oldIns = oldWidget.dataNote.drug_instruction?.toString() ?? '';
    final newIns = widget.dataNote.drug_instruction?.toString() ?? '';

    if (oldIns != newIns || oldWidget.typeCard != widget.typeCard) {
      setState(() {
        _isSaved = widget.dataNote.smw_transaction_order_id != null;
        localDataNote = widget.dataNote;
        txtComment.text = widget.dataNote.comment ?? '';
        localFiles = widget.dataNote.file ?? [];
  ...
  void dispose() {
    txtComment.dispose();
    txtRemarkImages.dispose();
    txtRemarkObs.dispose();
    txtNewComment.dispose();
    super.dispose();
  }

  // ---------------------------------------------
  // Your existing functions (initDoctors / loadFiles) คงไว้ได้
  // ---------------------------------------------
  ...
  Future<void> loadFiles() async {
    ...
    setState(() {
      ...
      localFiles.addAll(
        files.map(
            (f) => FileModel(path: f.path_file ?? '', remark: f.remark ?? '')),
      );
          ...
          child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      if (widget.typeCard == 'Food')
                        Visibility(
                          visible:
                              (widget.note.id == null || widget.note.id == 0) &&
                  ...
                  if (widget.typeCard == 'Food' || widget.typeCard == 'Drug')
                    text(
                      context,
                  ...
                  if (widget.typeCard == 'Food')
                    Wrap(
                      ...
                      children: prepOptions.map((label) {
                        final bool isSelected = selectedPrep[label] ?? false;
                        ...
                        return GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                color: const Color.fromARGB(255, 138, 119, 44),
                                width: 2,
                              ),
                            ),
                            child: text(
                  ...
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: levelList.isNotEmpty
                        ? levelList.map((level) {
                            final bool isSelected = selectedLevel == level;
                            return GestureDetector(
                              onTap: (_isSaved || isDisabled)
                                  ? null
                                  : () {
                  ...
                  (widget.typeCard == 'Food' || widget.typeCard == 'Drug')
                      ? FoodDrugCheckboxGroup(
                          ...
                          prePareStatus: widget.dataNote.pre_pare_status ?? '',
                          onChanged: (newValue) {
                  ...
                  if ((widget.dataNote.drug_type_name ?? '').trim() ==
                      '[I]ยาฉีด')
                    _isSaved && (widget.dataNote.doctor?.isNotEmpty ?? false)
                        ? Align(
                        ...
                        : GestureDetector(
                            onTap: () async {
                              ...
                              final result = await AdmitApi().loadDataDoctor(
                                context,
                                headers_: widget.headers,
                              );
                          ...
                          onTap: () async {
                            ...
                            if (filesForGrid.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content:
                                        Text('ไม่มีรูปภาพที่สามารถแสดงได้')),
                              );
                              return;
                            }
                          ...
                          child: Row(
                            children: [
                              ...
                              text(context, 'กดดูรูปภาพ',
                                  color:
                                      const Color.fromARGB(255, 13, 81, 116)),
                            ],
                          ),
                        ),
                      ],
                    ...
                    Align(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: isLoading ||
                            ...
                            : () async {
                                setState(() => isLoading = true);
                                final mPetAdmit = widget.lPetAdmit.firstWhere(
                                  (pet) => pet.hn == widget.note.smw_admit_id,
                                  orElse: () => widget.lPetAdmit.first,
                                );

                                final isChecked = (widget.dataNote.status ?? '')
                                    .trim()
                                    .isNotEmpty;

                                if (widget.dataNote.type_card != 'Observe' &&
                                    !isChecked) {
                                  setState(() => isLoading = false);
                                  AwesomeDialog(
                                    context: context,
                                ...
                                final mNoteDataDetail = CreateTransDetailModel(
                                  smw_admit_id: widget.note.smw_admit_id ?? 0,
                                  ...
                                  dataNote: [
                                    DataTransectionModel(
                                      smw_transaction_order_id: widget
                                          .dataNote.smw_transaction_order_id,
                                      smw_admit_order_id:
                                          widget.dataNote.smw_admit_order_id ??
                                              0,
                                      type_card:
                                          widget.dataNote.type_card ?? '',
                                      item_name:
                                          widget.dataNote.item_name ?? '',
                                      item_qty: widget.dataNote.item_qty ?? 0,
                                      unit_name:
                                          widget.dataNote.unit_name ?? '',
                                      dose_qty: widget.dataNote.dose_qty ?? '',
                                      meel_status:
                                          widget.dataNote.meal_timing ?? '',
                                      drug_instruction:
                                          widget.dataNote.drug_instruction ??
                                              '',
                                      ...
                                      drug_description:
                                          widget.dataNote.drug_description ??
                                              '',
                                      time_slot:
                                  ...
                                  btnOkOnPress: () async {
                                        ...
                                        : widget.dataNote.type_card != 'Observe' &&
                                            ...
                                            ? updatedNote.dataNote.firstWhere((e) =>
                                                ...
                                                e.doctor ==
                                                    widget.dataNote.doctor)
                                            : updatedNote.dataNote.firstWhere((e) =>
                                                e.item_code == widget.dataNote.item_code &&
                                                e.type_card ==
                                                    widget.dataNote.type_card &&
                                                e.doctor == widget.dataNote.doctor);
                                    // final updatedItem =
                                    ...
                                    setState(() {
                                      ...
                                      widget.dataNote.smw_transaction_order_id =
                                          updatedItem.smw_transaction_order_id;
                                      widget.dataNote.file = updatedItem.file;
