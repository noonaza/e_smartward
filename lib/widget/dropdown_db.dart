...
class DbSiteDropdown extends StatefulWidget {
  ...
  @override
  State<DbSiteDropdown> createState() => _DbSiteDropdownState();
}
...
class DbWardDropdown extends StatefulWidget {
  final String? selectedSiteCode;
  final Function(String) onSelected;

  const DbWardDropdown({
    super.key,
    required this.onSelected,
  ...
  @override
  State<DbWardDropdown> createState() => _DbWardDropdownState();
}

class _DbWardDropdownState extends State<DbWardDropdown> {
  ...
  @override
  void didUpdateWidget(covariant DbWardDropdown oldWidget) {
  ...
  void _loadWardData() async {
    if (widget.selectedSiteCode != null) {
      final ward = await DashboardApi().loadDataWard(
        context,
